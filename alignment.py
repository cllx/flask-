...
def list_to_newjson(word, res):
    ...
    elif word=='任职' or word=='任免_辞职' or word=='博士后':
        align_json['人物履历']['工作经历'][word]=res
    elif word=='学术兼职' or word=='技术工作兼职' or word=='社会兼职':
        align_json['其他职务'][word]=res
    elif word=='学术论著类' or word=='散文类' or word=='承担项目类' or word=='研究成果类' or word=='发明专利类':
        align_json["主要成果"][word]=res
    elif word=='教育理念' or word=='指导学生':
        align_json["主要成果"]["人才培养类"][word]=res
    else:
        align_json[word]=res
...
def preserve_key_queue(key, key_queue):
    if key=='任免_辞职' or key=='指导学生':
        delete = key_queue.get()
        delete = key_queue.get()
    elif key=='社会兼职' or key=='博士研究生':
        delete = key_queue.get()
    else:
        pass
...
def compare(content_list, index, baike_name):
    if index=='起始时间' or index=='结束时间' or index=='出生日期' or index=='时间点':
    ...
    if not st.is_empty():
        ...
        for i in attribute_value_list:
            ...
            if type(i).__name__ == 'list': #处理百科json文件中该属性下的值如果是列表时的情况，则取出列表里的每一个字典
                ...
                flag = 1
                if word=='基本信息' or word=='本科' or word=='硕士研究生' or word=='博士研究生' or word=='博士后' or word=='指导学生' or word=='教育理念':
...
if __name__ == "__main__":
    ...
    process_entrace(key_list, baike_name, yuanshi_name)