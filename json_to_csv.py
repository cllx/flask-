...
class json_to_csv():
	...
	'''
	def preserve_key_queue(self, key, key_queue):
		if key=='任免_辞职':
			...
			for i in attribute_value_list:
				if type(i).__name__ == 'list': #处理百科json文件中该属性下的值如果是列表时的情况，则取出列表里的每一个字典
					...
					if word=='人物影响' or word=='研究领域': #这两个属性的值没有子属性了，其值直接就是字符串形式，所以单独处理
						...
						res = self.process_list_no_same(word, i) #返回的是一个列表，列表里面是多个字典
						key = key_queue.get()
	...
	def main(self, filename):
		...
		global relation
		global count_id
		dirpath = 'static/xiaoqi_json/'+filename+'.json'
		...
		count_id = 1
		first_node = {'id':1,'name':yuanshi_name}
		node = pd.DataFrame(first_node,index=[0])
		first_relation = {'from_id':1,'pro1':'中文名','to_id':count_id}
		relation = pd.DataFrame(first_relation,index=[0])
		count_id += 1
		self.process_entrace(key_list, filename)
