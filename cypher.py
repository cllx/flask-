...
@app.route('/xiaoqi', methods=['GET', 'POST'])
def xiaoqi():
    if request.method == 'POST':
        filename = request.form['filename']
        filename = filename+'.json'
...
@app.route('/transe_generate_deal_csv', methods=['GET', 'POST'])
    ...
    if request.method == 'POST':
        ...
        for i in transeimg:
            ...
            if imgname==savedimg:
                ...
                return render_template('transe_entreace.html', transe_res=respath)
        json2csv = json_to_csv()
        json2csv.main(filename)
        respath = 'static/TransE_img/'+imgname
        return render_template('transe_entreace.html', transe_res=respath, tips='time_wait')
