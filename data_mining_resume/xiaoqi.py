	...
	for key, value in dictionary.items():
		...
		#成就
			...
			for key1 in dictionary[key]:
				if key1 == headline2['keyanfangxiang']:
					pass
				elif key1 == headline2['yanjiulingyu']:
					pass
				elif key1 == headline2['zhuyaorongyu']:
					for v in dictionary[key][key1]:
						...
						xiaoqi_list.append(name + ' ' + v[headline2['shijian']] + v[headline2['rongyu']]
							+ '|' + str(num) + ' '+ str(is_multi_value)
							+ ' ' + str(number))
				else:
...
def xiaoqi(name, merge_file):	
	...
	with open(path, 'w', encoding = 'utf-8') as file:
		json.dump(monitorItems, file, ensure_ascii = False, indent = 4)
		print("success")
	json2csv_operate_neo4j.main(xiaoqi_json)
	return xiaoqi_json
